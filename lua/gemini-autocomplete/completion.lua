  ...
  vim.api.nvim_create_autocmd('CursorMovedI', {
    ...
    callback = function()
      ...
      if util.is_blacklisted(blacklist_filetypes, filetype) or util.is_blacklisted(blacklist_filenames, filename) then
        util.notify("Gemini-autocomplete: Did not complete, file is blacklisted in config", vim.log.levels.WARN)
        return
      end
      M.gemini_complete()
    end,
...
M.prompt_code = function()
  ...
  api.gemini_generate_content(user_text, system_text, model_id, generation_config, function(response_lines)
    vim.schedule(function()
      ...
      util.notify('Done generating. Result inserted below cursor.', vim.log.levels.INFO)
